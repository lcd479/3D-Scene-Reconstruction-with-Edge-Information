...
class backbone(nn.Module):
    ...
    def __init__(self, img_size = (640,480)):
        ...
        self.SNet0  = SirenNet(dim_in = out_f[2], dim_hidden=256, dim_out= out_f[2], num_layers = 5,w0= 1.0, w0_initial=30.0, final_activation= None)
        self.SNet1  = SirenNet(dim_in = out_f[1], dim_hidden=256, dim_out= out_f[1], num_layers = 5,w0= 1.0, w0_initial=30.0, final_activation= None)
        self.SNet2  = SirenNet(dim_in = out_f[0], dim_hidden=256, dim_out= out_f[0], num_layers = 5,w0= 1.0, w0_initial=30.0, final_activation= None)
    ...
    def forward(self, x): 
        ...
        S_out0 = self.SNet0(self.conv0(out_list[0]).view(B, 24,-1).permute(0,2,1)).permute(0,2,1)
        S_out1 = self.SNet1(self.conv1(out_list[1]).view(B, 40,-1).permute(0,2,1)).permute(0,2,1)
        S_out2 = self.SNet2(self.conv2(out_list[2]).view(B, 80,-1).permute(0,2,1)).permute(0,2,1)
